...
namespace VOL.System.Services
{
    public partial class Sys_UserService
    {
        ...
        /// <param name="loginInfo"></param>
        /// <returns></returns>
        public async Task<WebResponseContent> Login(WindowsLoginInfo loginInfo)
        {
            string msg = string.Empty;
            ...
            {
                var result = md5.ComputeHash(Encoding.ASCII.GetBytes(loginInfo.userName + "SSMECT" + DateTime.Now.ToString("yyyyMMdd")));
                var strResult = BitConverter.ToString(result).Replace("-", "");

                if (strResult != loginInfo.passWord)
                {
                    return responseContent.Error("！验证失败");
                }
            }
            try
            {
                Sys_User user = await repository.FindAsIQueryable(x => x.UserName == loginInfo.userName && x.Enable == 1)
                    .FirstOrDefaultAsync();
